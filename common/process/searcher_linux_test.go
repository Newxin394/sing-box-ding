...
func TestProcessPathSnapshotIsThrottled(t *testing.T) {
	...
	first, loaded := linuxSearcher.processPathCache.Get(processPathsAllUsers)
	if !loaded {
	...
	second, loaded := linuxSearcher.processPathCache.Get(processPathsAllUsers)
	if !loaded {
...
// TestProcessPathCacheLifetimeOutlastsRescanInterval guards the coupling the
// throttle depends on. The cached snapshot has to stay usable for longer than
// the rescan interval: once the entry expires the cache lookup misses, the
// throttle branch is skipped entirely, and every lookup rebuilds again, which
// is the behaviour the throttle exists to prevent.
func TestProcessPathCacheLifetimeOutlastsRescanInterval(t *testing.T) {
	if processPathCacheLifetime <= processPathRescanInterval {
		t.Fatalf("cache lifetime %v must outlast the rescan interval %v", processPathCacheLifetime, processPathRescanInterval)
	}
}
