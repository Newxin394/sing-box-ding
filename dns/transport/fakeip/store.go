...
func (s *Store) Create(domain string, isIPv6 bool) (netip.Addr, error) {
	...
	if asyncStorage, ok := s.storage.(interface{ FakeIPSaveMetadataAsync(*adapter.FakeIPMetadata) }); ok {
		asyncStorage.FakeIPSaveMetadataAsync(metadata)
	} else if err := s.storage.FakeIPSaveMetadata(metadata); err != nil {
		return netip.Addr{}, E.Cause(err, "save fakeip metadata")
	}
	return address, nil
}
