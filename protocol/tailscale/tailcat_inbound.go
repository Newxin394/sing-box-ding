...
func (i *TailcatInbound) JudgeFlow(network uint8, source netip.AddrPort, destination netip.AddrPort, firstPacket []byte) tun.FlowVerdict {
	...
	return adapter.JudgeFlow(i.router, adapter.InboundContext{Inbound: i.Tag(), InboundType: i.Type(), User: i.node.userName(source.Addr())}, network, source, destination, firstPacket)
}
